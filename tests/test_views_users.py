...
async def test_put_user_update_200_ok(
    ...
    res = await get_client.put(
        get_app.url_path_for("users:put", user_id=user["id"]), content=new_user.json()
    )
    assert res.status_code == status.HTTP_200_OK
    ...
    if not data.get("password") is None:
        assert password_hash_ctx.verify(
            data.get("password"), res.json().get("password")
        )
    assert "created" in res.json()
    assert "last_login" in res.json()
...
async def test_put_user_update_404_not_found(get_client: AsyncClient, get_app: FastAPI):
    ...
    res = await get_client.put(
        get_app.url_path_for("users:put", user_id="9999"), content=new_user.json()
    )
    assert res.status_code == status.HTTP_404_NOT_FOUND
...
async def test_patch_user_update_404_not_found(
    ...
    new_user = UserUpdate(**user)
    res = await get_client.patch(
        get_app.url_path_for("users:patch", user_id="9999"), content=new_user.json()
    )
    assert res.status_code == status.HTTP_404_NOT_FOUND
    assert res.json() == {"detail": f"User with id '9999' not found"}
